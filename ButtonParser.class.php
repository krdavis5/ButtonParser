   ...
   public static function renderButtonLink( $parser ) {
      ...
      if(substr($options['link'], 0, 4) == "http") {
        ...
        if(empty($options['title'])) {
          ...
          $output = "<a class=\"mw-ui-button mw-ui-progressive\" href=\"" . $options['link'] . "\">" . $options['link'] . "</a>";
        } else {
          // We apparantly have a title present
          $output = "<a class=\"mw-ui-button mw-ui-progressive\" href=\"" . $options['link'] . "\">" . $options['title'] . "</a>";
        }
